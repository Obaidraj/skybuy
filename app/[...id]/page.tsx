...
import { addToWishlist, addtoCart, removeFromWishlist } from '@/redux/slices/common/commonSlice';
...
const additemCart=()=>{
  ...
  return (
   ...
   {loadingProductdetail? <Lottie animationData={Loading} loop={true} className="h-[45%]"/>: <>
    ...
    />
        ...
        <div className="grid grid-cols-4 gap-1 h-[28dvh] overflow-y-auto">
          {productDetail?.images?.map((image) => (
            <div key={image} className=" border hover:border-primary h-full cursor-pointer" onClick={()=>ChangeMianImage(image)}>
              <Image
                src={image}
                alt="thumbnail"
                height={700}
      ...
      <div className="md:w-[50%] w-full flex flex-1 flex-col ">
        <div className='flex gap-2 '><div className='flex'>   {productDetail?.rating&&[...Array(Math.floor(productDetail?.rating))].map((_, index) => (
              <svg
                key={index}
                aria-hidden="true"
            ...
            ))}<span className='text-gray-500 px-1'>{productDetail?.rating?.toFixed(1)} Review(s)</span> </div> </div>
...
<div> <span className='font-bold'>Brand:</span>  {productDetail.brand}</div>
<div> <span className='font-bold'>Category:</span>  {productDetail.category}</div>
<div> <span className='font-bold'>Discount:</span>  {productDetail.discountPercentage}%</div>
<div> <span className='font-bold'>Price:</span>  ${productDetail.price}</div>
<div> <span className='font-bold'>Available In Stock:</span>  {productDetail.stock}</div>

        </div>
        <div className='flex justify-start flex-row gap-2'>
          <div className='w-[10%]'>

           <Input placeholder='0' type='number' handleChange={handleChange} value={items}/>
         </div>
           <div>
          ...
          <div className='flex my-2' >
            <div className='hover:text-primary cursor-pointer flex items-center text-xl' onClick={e=>handleClick(e)}>

         {wishlist.includes(productDetail)?<><FaHeart className="text-primary px-2"/> Remove from Wishlist</> :<><CiHeart /> Add to Wishlist</> }
            </div>
          </div>
